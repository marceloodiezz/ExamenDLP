...
public class TypeCheckingVisitor extends AbstractVisitor<Void, Type> {
    ...
    @Override
    public Void visit(TernaryOp t, Type param) {
        super.visit(t, param);

        // La condición debe ser lógica
        t.getCondition().getType().mustBeLogical(t);

        // Calcular tipo común entre rama True y rama False
        Type commonType = t.getExprTrue().getType().ternary(t.getExprFalse().getType(), t);
        t.setType(commonType);
